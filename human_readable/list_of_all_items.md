# All items

Every item currently in `Assets/Resources/Data/Items/`, sorted by gear
slot (in `GearSlot` enum order). Regenerate/update this by hand whenever
items are added, renamed, or retuned — it's a snapshot, not auto-generated.

## Helmet

| Item | Effect |
|---|---|
| Champion's Helmet | +50 Max Health, +5 Armor, +10% threat generated |

## Necklace

| Item | Effect |
|---|---|
| Amulet of Vitality | +150 Max Health, +5 Armor |
| Amulet of the Magi | +100 Max Mana, +4 mana per 5s |
| Amulet of the Berserker | +5 Weapon Damage |

## Chest

| Item | Effect |
|---|---|
| Barbarian's Mantle | Above 50% HP: −15% damage taken. Below 50% HP: +15% damage dealt. |

## Cape

| Item | Effect |
|---|---|
| Champion's Cloak | +20% threat generated, +50 Max Health |

## Boots

| Item | Effect |
|---|---|
| Armored Boots | +5 Armor |
| Boots of Lightness | Press Jump again while airborne to suspend falling for 2s, freely steerable with WASD — once per airtime |
| Ice Cleats | Immune to Slow from ground patches |
| Swift Boots | +3% Run Speed |

## Ring 1 / Ring 2

| Item | Effect |
|---|---|
| Transmitting Beacon | You always show on allies' minimaps |

## Trinket

| Item | Effect |
|---|---|
| Ember Stone | +10% damage dealt; wearer is permanently Burning |
| The Everflow | +10 mana per 5s |

## Main Hand

| Item | Weapon | Effect |
|---|---|---|
| Broad Sword | 40 damage / 1.5s swing, 2 range | +40% threat generated, +100 Max Health |
| Staff | 20 damage / 2s swing, 2 range | −10% mana cost, +250 Max Mana. Two-handed |
| 2H Axe | 80 damage / 3s swing, 2 range | Two-handed, no other bonus |
| Holy Scepter | none | +10% healing done |
| Hunter's Bow | 60 damage / 2.5s swing, 50 range | No other bonus |
| Armorbreaker | 40 damage / 1.5s swing, 2 range | On hit, reduces the target's Armor by 2 for 10s, stacking up to 5 times |

## Off Hand

| Item | Effect |
|---|---|
| Aegis of the Unstoppable | +20 Armor, +5% chance to block melee attacks |
| Aegis of Reflection | +10 Armor, +5% chance to block melee attacks, reflects 3% of incoming damage back at the attacker |
| Tomb of the Magi | +50 Max Mana, +4 mana per 5s |

## Slots with nothing yet

Gloves, Legs.
